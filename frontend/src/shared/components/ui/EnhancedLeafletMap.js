...
const EnhancedLeafletMap = ({ 
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      try {
        ...
        if (checkMobile) {
          ...
          if (mapRef.current && (mapRef.current.style.height === 'auto' || !mapRef.current.style.height)) {
            mapRef.current.style.height = '400px';
          }
          if (mapRef.current && (mapRef.current.style.width === 'auto' || !mapRef.current.style.width)) {
        ...
        if (!mapRef.current || mapRef.current.offsetWidth === 0 || mapRef.current.offsetHeight === 0) {
          ...
          const retryDelay = checkMobile ? 300 : 100;
        ...
        const map = L.map(mapRef.current, {
          ...
          renderer: checkMobile ? L.svg() : L.canvas(),
          zoomAnimation: !checkMobile, // Disable animations on mobile
          fadeAnimation: !checkMobile,
          markerZoomAnimation: !checkMobile,
        ...
        // Force multiple resizes to ensure proper rendering on mobile
        // Reuse checkMobile from earlier in the function
        if (checkMobile) {
    ...
    const initDelay = isMobile ? 300 : 200;
    const timer = setTimeout(initMap, initDelay);
  ...
  // Adjust height based on mobile state
  const mobileHeight = isMobile ? '400px' : height;
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        backgroundColor: '#e5e5e5', // Match CSS background
        position: 'relative',
        minHeight: isMobile ? '300px' : '300px', // Ensure minimum height to prevent dimension issues
