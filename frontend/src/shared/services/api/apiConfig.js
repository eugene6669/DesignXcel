...
class ApiConfig {
  constructor() {
    // Environment settings - set first so we can use it below
    ...
    // Force direct backend connection in development OR if running on localhost
    // This allows production builds to work locally too
    this.isLocalhost = typeof window !== 'undefined' && 
      (window.location.hostname === 'localhost' || 
       window.location.hostname === '127.0.0.1' ||
       window.location.hostname === '');
    
    if (this.environment === 'development' || this.isLocalhost) {
    ...
    if (this.environment === 'development' || this.debugMode) {
      console.log('🔧 API Configuration:', {
        ...
        detectedFrom: this.isLocalhost ? 'localhost detection' : 'environment variable'
      });
    }
