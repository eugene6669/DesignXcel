          ...
          if (status === 401) {
            ...
            if (isCustomerEndpoint) {
              ...
              if (error.config?.url?.includes('/api/auth/status')) {
                ...
                // Don't redirect - let the auth hook handle the logout
                return Promise.reject(error);
              }
              
              // For customer endpoints, check if it's an account endpoint that shouldn't redirect
              // Don't redirect on account page endpoints - let the component handle it
              const isAccountEndpoint = error.config?.url?.includes('/api/customer/orders') || 
                                        error.config?.url?.includes('/api/customer/addresses') ||
                                        error.config?.url?.includes('/api/customer/profile');
              
              if (!isAccountEndpoint) {
                // For critical customer endpoints, redirect to login
                console.log('🔒 401 error for customer endpoint - redirecting to login');
                if (window.location.pathname !== '/login') {
                  window.location.href = '/login';
                }
              } else {
                // For account endpoints, don't redirect - let the component handle it
