        ...
        try {
            if (isLogin) {
                ...
                if (result.success) {
                    ...
                    // Check if user had checkout intent (was on checkout page before login)
                    ...
                    if (from.startsWith('/admin')) {
                        ...
                        if (result.user.role === 'Admin' || result.user.role === 'Employee') {
                            ...
                            // If user was on checkout and has cart items, redirect to cart
                            redirectTo = '/cart';
                            localStorage.removeItem('checkout-intent'); // Clear intent after use
                        ...
                        // If checkout intent exists and user has cart items, redirect to cart
                        redirectTo = '/cart';
                        localStorage.removeItem('checkout-intent'); // Clear intent after use
                    }
                    ...
                    if (result.code === 'INCORRECT_PASSWORD' || result.error?.toLowerCase().includes('incorrect password')) {
