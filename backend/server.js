...
app.post('/api/stripe/webhook', bodyParser.raw({ type: 'application/json' }), async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        try {
            ...
            const existingRegularOrder = await findExistingOrderByCheckoutSessionId(pool, session.id);
            if (existingRegularOrder) {
                console.warn(
                    '[STRIPE WEBHOOK] Order already exists for session:',
                    session.id,
                    'OrderID:',
                    existingRegularOrder.OrderID
                );
                return res.status(200).json({
                    received: true,
                    message: 'Order already exists, skipped duplicate',
                    orderId: existingRegularOrder.OrderID
                });
            }
    ...
    try {
        ...
        const ordersResult = await pool.request()
            ...
            .query(`
                ...
                ORDER BY o.OrderDate DESC
            `);
        const orders = dedupeOrdersByCheckoutSession(ordersResult.recordset);
    ...
    try {
        ...
        if (event.type === 'checkout.session.completed') {
            ...
            try {
                ...
                const existingTestWebhookOrder = await findExistingOrderByCheckoutSessionId(pool, session.id);
                if (existingTestWebhookOrder) {
                    console.log('[TEST WEBHOOK] Order already exists with session ID:', session.id);
                    return res.status(200).json({
                        success: true,
                        orderId: existingTestWebhookOrder.OrderID,
                        message: 'Order already exists'
                    });
                }
