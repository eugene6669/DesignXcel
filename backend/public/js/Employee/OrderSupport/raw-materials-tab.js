(function () {
    ...
    function initQuickAdd() {
        ...
        submitBtn.addEventListener('click', async function () {
            ...
            for (const cb of checked) {
                ...
                try {
                    const res = await fetch('/Employee/OrderSupport/RawMaterials/Add', {
                        method: 'POST',
    ...
    function initRawMaterialFormsRealtime() {
        const addForm = document.querySelector('#rmAddMaterialModal form[action="/Employee/OrderSupport/RawMaterials/Add"]');
        ...
        if (editForm && !editForm.getAttribute('data-rm-realtime')) {
            ...
            editForm.addEventListener('submit', async function (e) {
                ...
                try {
                    ...
                    const res = await fetch('/Employee/OrderSupport/RawMaterials/Edit', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                        credentials: 'include',
                        body: formData
